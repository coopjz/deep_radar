...
class Alarm(CompeteMap):
    ...
    def two_camera_merge_update(self, locations, extra_locations, depth_predictor, 
                                use_save_radar=True, input_imgs=None):
        """
        ...
        Modify this method to perform update on two cameras only
        """
        if self._two_camera:
            # init location
            for i in range(1, 11):
            ...
            rls = []
            ex_rls = []
            for location, e_location, dp, input_img in zip(locations, extra_locations, depth_predictor, input_imgs):
                # 针对每一个相机产生的结果

                if (isinstance(location, np.ndarray) or isinstance(e_location, np.ndarray)) and (not use_save_radar):
                    depth = dp.infer(input_img)

                # 对于用神经网络直接预测出的装甲板，若不为None
                if isinstance(location, np.ndarray):
                    # z is the mean of the depth of 2 x armor area
                    # depth is from livox to camera coordinate and then to the real world
                    if use_save_radar:
                        l = dp.detect_depth(location[:, 11:])  # (N,x0+y0+z)  z maybe nan
                        # print(l)
                        # exit()
                    else:
                        l = dp.detect_depth(location[:, 11:], depth)
                        l[:,2] = l[:,2]*7
                        # print(l)
                        # exit()
                    # nan滤除
                ...
                if isinstance(e_location, np.ndarray):
                    ...
                    if use_save_radar:
                        e_l = dp.detect_depth(e_location[:, 1:])
                    else:
                        e_l = dp.detect_depth(e_location[:, 1:], depth)
                        e_l[:,2] = e_l[:,2]*7
                    # nan滤除
            ...
            for armor in self._ids.keys():
                ...
                if isinstance(rls[0], np.ndarray):
                    ...
                    if mask.any():  # 若有
                        ...
                        # print(l1)
                        # print(np.concatenate([l1[1:3], np.ones(1)], axis=0) * l1[3])
                        # print(np.concatenate([np.concatenate([l1[1:3], np.ones(1)], axis=0) * l1[3], np.ones(1)], axis=0))
                        # 坐标换算为世界坐标
            ...
            for i in range(1, 11):
                ...
                # print("{}: {}".format(str(i), location[str(i)]))
