...
from radar_class.depth_predictor import DepthPredictor
...
class MainProcess(object):
    def __init__(self, myshow: Mywindow, log_f):
        ...
        if using_video and (not use_hik):
            # cap 1 is the right
            # cap 2 is the left
    ...
    def spin_once(self, base_open=False, base_close=False, whole_close=False, is_opening=False):
        ...
        '''
        # check radar init
        if (not using_dq) and use_save_radar:
...
if __name__ == "__main__":
    ...
    try:
        ...
        key_close = False
        last_judge_update_time = time.time()

        message = dict()
        message['task'] = 1
        message['data'] = {}
        for i in range(1, 11):
            message['data'][str(i)] = [0, 0]
        while True:
            t1 = time.time()
            # if (time.time() - last_judge_update_time) > 0.5:
            #     print("--> Referee system update")
            #     send_judge(message, None, None)
            #     last_judge_update_time = time.time()
